...
import type { SchedulesData } from "@/api/schedules/schedules.types";
...
const RegisterSchedules = ({ route }: CreateWorkoutProps) => {
	...
	const defaultValuesById = useMemo(() => {
		...
		return {
			time: originalTime.includes("Personalizado")
				? ["Personalizado"]
				: originalTime,
	...
	const isUserSubscribed = scheduleById?.students?.some(
		(item) => item.studentId === user?.id,
	);
	const buttonLabel = isUserSubscribed
		? "Cancelar inscrição"
		: "Quero me inscrever";
	...
	return (
		<>
			<Appbar.Header>
				<Appbar.BackAction
					onPress={() => navigation.navigate("SchedulesStudent" as never)}
				/>
						...
						renderItem={() => (
							<Card mode="outlined">
								<View style={{ padding: 16 }}>
									<Card.Title
										title={scheduleById?.name}
										subtitle={scheduleById?.description}
									...
									<Card.Content style={{ padding: 16, gap: 16 }}>
										...
										{(scheduleById?.students?.length || 0) >=
											(scheduleById?.studentLimit || 1) && (
											<CustomChip
												color="error"
												label={"Capacidade da turma atingida"}
												icon="information"
												style={{
													marginVertical: 12,
													padding: 10,
										...
										<Button
											...
											disabled={
												mutation.isPending ||
												((!isUserSubscribed &&
													scheduleById?.students?.length) ||
													0) >=
													((!isUserSubscribed && scheduleById?.studentLimit) ||
														1)
											}
