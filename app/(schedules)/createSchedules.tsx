...
const CreateSchedules = ({ route }: CreateWorkoutProps) => {
	...
	const defaultValuesById = useMemo(() => {
		...
		const defaultOnly = originalTime.filter(
			(t) => timeOptionValues.has(t) && t !== "Personalizado",
		);

		return {
			time: originalTime.includes("Personalizado")
				? [...defaultOnly, "Personalizado"]
				: originalTime,
					...
					<FlatList
						...
						renderItem={() => (
							<View>
								...
								{activeTab === "students" && (
									<>
										{(getValues().students?.length || 0) >
											(getValues().studentLimit || 1) && (
											<CustomChip
												color="error"
												label={`Ultrapassou o limite | Máx. ${getValues().studentLimit || 1} aluno(s)`}
												icon="information"
												style={{
													marginVertical: 12,
													padding: 10,
												...
												<View style={{ marginBottom: 10 }}>
													...
													>
														{studentsSelected?.map((student) => (
															<Chip
																key={student.studentId}
																onClose={() => handleRemove(student.studentId!)}
															>
